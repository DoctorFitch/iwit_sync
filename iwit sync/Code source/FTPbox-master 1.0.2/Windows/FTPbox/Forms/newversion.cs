    ...
    {        
        ...
        private void bDownload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DialogResult dr = MessageBox.Show("Le fichier updater.exe est manquant dans le dossier. S'il vous plait, remettez le en place ou réinstallé le programme.", "iwit sync - Fichier manquant", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error);
                    if (dr == DialogResult.Cancel)
                        Process.GetCurrentProcess().Kill();
                }

                var param = string.Format("{0} {1} {2}", "iwit sync", newvers, downLink);
        ...
        private void bLearnMore_Click(object sender, EventArgs e)
        {
            try
            {
                Process.Start("http://iwit-systems.fr/");
            }
            catch { }
            Close();
        }
        ...
        private void Set_Language(string lan)
        {
            ...
            Text = "iwit sync | " + Common.Languages[UiControl.UpdateAvailable];
