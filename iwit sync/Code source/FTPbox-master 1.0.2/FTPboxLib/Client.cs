 ...
 * - Copyright (C) 2012-2013 ftpbox.org
    ...
    public class Client
    {
                ...
                if (FTP)
                {
                    using (Stream file = File.Open(i.LocalPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite), rem = _ftpc.OpenWrite(temp))
                    {
                        ...
                        while ((read = file.Read(buf, 0, buf.Length)) > 0)
                        {
                            ...
                            ThrottledStream destinationStream = new ThrottledStream(rem, Settings.General.UploadLimit * 1024);

                            byte[] buffer = new byte[8192];
                            int readCount = file.Read(buffer, 0, 8192);

                            while (readCount > 0)
                            {
                                destinationStream.Write(buffer, 0, readCount);
                                readCount = file.Read(buffer, 0, 8192);
                            }
        ...
        public TransferStatus SafeDownload(SyncQueueItem i)
        {
            ...
            if (i.Item.Size == new FileInfo(temp).Length)
            {
                ...
                if (File.Exists(i.LocalPath))
                    ...
                    #else
                    FileIO.FileSystem.DeleteFile(i.LocalPath, FileIO.UIOption.OnlyErrorDialogs, FileIO.RecycleOption.DeletePermanently);
                    #endif
        ...
        Finish:
            if (File.Exists(temp))
                ...
                File.Delete(temp);
                #else
                FileIO.FileSystem.DeleteFile(temp, FileIO.UIOption.OnlyErrorDialogs, FileIO.RecycleOption.DeletePermanently);
                #endif
            return TransferStatus.Failure;
        }
        ...
        private void ThrottleTransfer(int limit, long transfered, DateTime startedOn)
        {
            ...
            if (limit > 0 && rate > 1000 * limit)
            {
                ...
                int millisecDelayConvert = (int)millisecDelay;

                Thread.Sleep(millisecDelayConvert);
             }
            */
            /*
                // test
                ...
                if (Elapsed > 5000)
                {
                    ...
                    int TransmitDelta = (int)transmitted - ExpectedTransmit;
                    if (TransmitDelta > 0)
                        Thread.SpinWait(TransmitDelta * OneSecond / SpeedLimit);
                    Watch.Reset();
                }
                Watch.Stop();
             */

            // On obtient le nombre maximum de bytes par seconde
