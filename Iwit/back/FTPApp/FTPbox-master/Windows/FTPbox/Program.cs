...
namespace FTPbox
{
    static class Program
    {
        ...
        private static void RunClient(string[] args, string param)
        {
            if (!isServerRunning)
            {
                MessageBox.Show("iWiTeXchange doit fonctionner pour utiliser le menu contextuel!", "iWiTeXchange", MessageBoxButtons.OK, MessageBoxIcon.Information);
                RemoveFTPboxMenu();
                Process.GetCurrentProcess().Kill();
            }
            
            NamedPipeClientStream pipeClient = new NamedPipeClientStream(".", "iWiTeXchange Server", PipeDirection.InOut, PipeOptions.None, System.Security.Principal.TokenImpersonationLevel.Impersonation);

            Log.Write(l.Client, "Connecting client...");
            pipeClient.Connect();

            StreamString ss = new StreamString(pipeClient);
            if (ss.ReadString() == "iWiTeXchange")
            {
        ...
        private static bool isServerRunning
        {
            get
            {
                ...
                return processes.Any(p => p.ProcessName == "iWiTeXchange" && p.Id != Process.GetCurrentProcess().Id);
            }
        }
                ...
                if (allprocesses.Length > 0)
                    foreach (Process p in allprocesses)
                        if (p.Id != Process.GetCurrentProcess().Id)
                        {
                            ...
                            if (!p.HasExited)
                            {
                                MessageBox.Show("iWiTeXchange est déjà en cours de fonctionnement.", "iWiTeXchange",
